...
assert.match(loginPage, /:deep\(\.patient-login-card \.el-card__header\)\s*\{[\s\S]*padding:\s*28px var\(--login-shell-inline\) 18px;/);
...
assert.doesNotMatch(loginPage, /<el-alert\s+v-if="!patientModeEnabled"/);
